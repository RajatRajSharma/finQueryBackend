...
def test_evaluation_service_builds_rich_run_and_caches(tmp_path):
    ...
    # Per-question row merges the pipeline output with source metadata.
    q0 = run.questions[0]
    ...
    # Cached + reloads + is fresh within the TTL.
...
def test_evaluation_cached_ignores_malformed_or_old_format_file(tmp_path):
    service = _eval_service(tmp_path)
    # An old-schema / malformed cache must not 500 the endpoint — treat as "no run".
...
def test_evaluation_cache_goes_stale_past_ttl(tmp_path):
    service = _eval_service(tmp_path)
    run = service.run()
    # Hand a run with an old timestamp: should be stale.
...
def test_evaluation_run_as_baseline_then_shows_in_next_run(tmp_path):
    ...
    run = service.run()                  # subsequent run picks it up
...
def test_evals_endpoint_run_then_get(tmp_path):
    ...
    try:
        ...
        # Nothing cached yet -> 404 with a helpful hint.
        assert client.get("/evals").status_code == 404
        # Trigger a (background) run; TestClient executes the task before returning.
        assert client.post("/evals/run").status_code == 202
        # Now the cached run comes back in camelCase, with per-question detail.
...
def test_gemini_pool_rotates_to_next_key_on_quota():
    ...
    def call(_client):
        calls["n"] += 1
        if calls["n"] == 1:        # key 1 is quota-exhausted...
            raise _quota_error()
